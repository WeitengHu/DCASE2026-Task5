...
def build_instruction(question: str, choices: list[str]) -> str:
    ...
    prompt += "Please think and reason step by step about the audio before you respond. After your reasoning, the final answer must match exactly one of the listed choices."
    return prompt
...
def load_model_and_processor(
    ...
    if lora_path:
        ...
        for adapter_name, cfg in model.peft_config.items():
            ...
            print("use_rslora:", getattr(cfg, "use_rslora", False))
            print("alpha_pattern:", getattr(cfg, "alpha_pattern", None))
            print("scaling factor:", cfg.lora_alpha / cfg.r)
...
def save_raw_output_csv(output_csv_path: str, rows: List[Dict]):
    ...
    with open(output_csv_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["question", "raw_output", "instruction"])
        writer.writeheader()
        for row in rows:
            writer.writerow(
                {
