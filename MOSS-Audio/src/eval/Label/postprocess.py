...
def extract_final_part(raw_output: str) -> Optional[str]:
    if not raw_output:
        return None
...
def build_labels(choices: List[str]) -> List[str]:
    return [chr(ord("A") + index) for index in range(min(len(choices), 26))]


def extract_choice_label(final_part: str, valid_labels: List[str]) -> Optional[str]:
    if not final_part or not valid_labels:
        return None

    text = str(final_part)
    final_part = clean_candidate_text(text).upper()

    label_chars = "".join(re.escape(label) for label in valid_labels)
    pattern = r"\b(" + "|".join(re.escape(label) for label in valid_labels) + r")\b"
    matches = re.findall(pattern, final_part)
    if matches:
        return matches[-1]

    dynamic_pattern = rf"(?<![A-Z])([{label_chars}])(?![A-Z])"
    matches = re.findall(dynamic_pattern, final_part)
    if matches:
        return matches[-1]

    for label in reversed(valid_labels):
        if re.search(rf"[\(\[\{{<\s]*{re.escape(label)}[\)\]\}}>:\.\s]*$", final_part):
            return label

    fallback_pattern = rf"\b([{label_chars}])\b"
    matches = re.findall(fallback_pattern, final_part)
    if matches:
        return matches[-1]
...
def label_to_choice(label: Optional[str], choices: List[str]) -> Optional[str]:
    if label is None:
        return None
            ...
            raw_outputs.append(
                {
                    ...
                    "choices": parse_choices(row.get("choices") or ""),
                }
            )
    return raw_outputs
...
def postprocess(
    ...
    for row_index, item in enumerate(raw_outputs):
        ...
        choices = item.get("choices") or dataset_choices.get(question_id, [])
        final_part = extract_final_part(raw_output)
        ...
        if final_part is not None:
            pred_label = extract_choice_label(final_part, build_labels(choices))
            matched_choice = label_to_choice(pred_label, choices)
            rows.append({"question": question_id, "answer": matched_choice or "None"})
            continue
